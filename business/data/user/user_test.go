...
func TestUser(t *testing.T) {
	...
	t.Log("Given the need to work with User records.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single User.", testID)
		{
			...
			saved, err := u.RetrieveByID(ctx, traceID, claims, usr.ID)
			if err != nil {
			...
			if diff := cmp.Diff(usr, saved); diff != "" {
				t.Fatalf("\t%s\tTest %d:\tShould retrieve the same user. Diff:\n%s", tests.Failed, testID, diff)
			}
			t.Logf("\t%s\tTest %d:\tShould retrieve the same user.", tests.Success, testID)

			upd := user.UpdateUser{
				Name:  tests.StringPointer("Janko Hrasko"),
				Email: tests.StringPointer("janko@hrasko.com"),
			}
			...
			saved, err = u.RetrieveByEmail(ctx, traceID, claims, *upd.Email)
			if err != nil {
			...
			_, err = u.RetrieveByID(ctx, traceID, claims, usr.ID)
