...
func (ug userGroup) retrieve(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	users, err := ug.user.Retrieve(ctx, v.TraceID, pageNumber, rowsPerPage)
	if err != nil {
...
func (ug userGroup) retrieveByID(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	v, ok := ctx.Value(web.KeyValues).(*web.Values)
	if !ok {
		return web.NewShutdownError("web value missing from context")
	}
	...
	params := web.Params(r)
	usr, err := ug.user.RetrieveByID(ctx, v.TraceID, claims, params["id"])
	if err != nil {
		switch err {
